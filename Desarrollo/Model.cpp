...
// Leer la información como indica ASSIMP
void Model::LoadModel(const std::string & fileName)
{
	...
	//const aiScene *scene=importer.ReadFile(fileName,aiProcess_Triangulate |aiProcess_FlipUVs|aiProcess_GenSmoothNormals|aiProcess_JoinIdenticalVertices); // Dos vértces con los mismo datos los pone en un solo vértice
	const aiScene *scene = importer.ReadFile(fileName, aiProcess_Triangulate | aiProcess_FlipUVs); 
	/*
	// Para evitar problemas a la hora de importar modelos
...
void Model::ClearModel()
{
	...
	{
		if (TextureList[i]) // arreglo de texturas
		{
...
void Model::RenderModel()
{
	for (unsigned int i = 0; i < MeshList.size(); i++)
	{
		...
		MeshList[i]->RenderMesh(); // se manda el RendeMesh
...
{
	// Carga información de vértices, normales, texturas y caras
	...
	for (unsigned int i = 0; i < scene ->mNumMaterials; i++)
	{
		...
		if (material->GetTextureCount(aiTextureType_DIFFUSE	))
		{
			...
			if (material->GetTexture(aiTextureType_DIFFUSE,0,&path)==AI_SUCCESS)
			{
				...
				std::string texPath = std::string("Textures/") + filename; //buscar en la carpeta Textures
		...
		if (!TextureList[i])
		{
			...
			TextureList[i]->LoadTextureA(); // Es una imagen blanca para los que no tengan textura
		}
